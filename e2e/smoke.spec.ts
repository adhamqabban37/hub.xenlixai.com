...
test("homepage loads and renders hero", async ({ page }) => {
  ...
  await expect(page.locator("text=XENLIX")).toBeVisible();
...
test("all navigation links are present and clickable", async ({ page }) => {
  await page.goto("/");
  const links = ["/", "/platforms", "/ai-tools", "/technology", "/about", "/resources", "/contact"];
  for (const href of links) {
    const label = href === "/" ? "Home" : href.replace(/\//g, "");
...
test("platforms page renders", async ({ page }) => {
  ...
  await expect(page.locator("text=XenlixAI Core")).toBeVisible();
});
...
test("voice demo section is present on homepage", async ({ page }) => {
  await page.goto("/");
  await expect(page.locator("text=Voice Agent")).toBeVisible({ timeout: 10000 });
});
